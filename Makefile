...
.PHONY: all build build-a2a-bridge install test test-fast vet lint compat-literals check-authz-guards check-setenv-guard check-custom golangci-lint web web-typecheck web-test fmt fmt-check ci ci-full clean help container-sciontool container-scion container-binaries proto proto-check
...
check-custom: compat-literals check-authz-guards check-setenv-guard
