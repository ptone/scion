...
type Store interface {
	...
	UpdateGuildName(ctx context.Context, guildID, guildName string) error
...
type ChannelLink struct {
	ChannelID          string
	GuildID            string
	GuildName          string // human-readable server name, populated at link time
	ProjectID          string
	ProjectSlug        string
	DefaultAgent       string
...
func (s *sqliteStore) UpdateGuildName(ctx context.Context, guildID, guildName string) error {
	_, err := s.db.ExecContext(ctx, `UPDATE channel_links SET guild_name = ? WHERE guild_id = ?`, guildName, guildID)
	return err
}
