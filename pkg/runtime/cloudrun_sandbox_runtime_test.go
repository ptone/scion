...
// newTestRuntime creates a CloudRunSandboxRuntime with a mock binary and
// temp state file, suitable for testing deleteWithTimeout.
func newTestRuntime(t *testing.T, bin string) *CloudRunSandboxRuntime {
	t.Helper()
	stateFile := filepath.Join(t.TempDir(), "state.json")
...
func TestCloudRunSandboxDeleteWithTimeout_NormalCompletion(t *testing.T) {
	// Use a binary that exits immediately.
	tmpDir := t.TempDir()
	mockBin := filepath.Join(tmpDir, "sandbox")
	script := "#!/bin/sh\nexit 0\n"
	if err := os.WriteFile(mockBin, []byte(script), 0755); err != nil {
	...
	if entry := rt.state.get("sb-ok"); entry != nil {
		t.Error("state entry still present after deleteWithTimeout()")
	}
}

func TestCloudRunSandboxDeleteWithTimeout_TimeoutTreatedAsSuccess(t *testing.T) {
	// Use "sleep 60" as a stand-in for a hanging process.
	rt := newTestRuntime(t, "sleep")
	rt.bin = "sleep" // the "binary" is sleep
	rt.deleteTimeout = 200 * time.Millisecond

	// We need a real binary that hangs. Use exec.Command("sleep", "60")
	// by setting the binary to a wrapper script that ignores args and sleeps.
	tmpDir := t.TempDir()
	mockBin := filepath.Join(tmpDir, "sandbox")
	script := "#!/bin/sh\nsleep 60\n"
	if err := os.WriteFile(mockBin, []byte(script), 0755); err != nil {
	...
	// Should complete in roughly the timeout duration, not 60 seconds.
...
func TestCloudRunSandboxDeleteWithTimeout_ProcessErrorNonFatal(t *testing.T) {
	// Use a binary that exits with an error — deleteWithTimeout should
	// still succeed (log warning but return nil).
	tmpDir := t.TempDir()
	mockBin := filepath.Join(tmpDir, "sandbox")
	script := "#!/bin/sh\nexit 1\n"
	if err := os.WriteFile(mockBin, []byte(script), 0755); err != nil {
...
func TestCloudRunSandboxDeleteWithTimeout_CancelsWatcher(t *testing.T) {
	// Verify that deleteWithTimeout cancels any registered watcher.
	tmpDir := t.TempDir()
	mockBin := filepath.Join(tmpDir, "sandbox")
	script := "#!/bin/sh\nexit 0\n"
	if err := os.WriteFile(mockBin, []byte(script), 0755); err != nil {
	...
	err := rt.deleteWithTimeout(context.Background(), "sb-watch")
	if err != nil {
		t.Fatalf("deleteWithTimeout() error = %v", err)
	}
	...
	rt.watchMu.Unlock()
	if present {
		t.Error("watcher cancel still in map after deleteWithTimeout()")
	}
}

func TestCloudRunSandboxDeleteWithTimeout_DefaultTimeout(t *testing.T) {
	// Verify that deleteTimeout=0 falls back to DefaultDeleteTimeout.
	tmpDir := t.TempDir()
	mockBin := filepath.Join(tmpDir, "sandbox")
	script := "#!/bin/sh\nexit 0\n"
	if err := os.WriteFile(mockBin, []byte(script), 0755); err != nil {
...
func TestKillProcessGroup_NilProcess(t *testing.T) {
	// killProcessGroup should not panic with a nil Process.
	cmd := &exec.Cmd{}
	// Should not panic.
...
func TestKillProcessGroup_RunningProcess(t *testing.T) {
	// Start a process in its own process group (matching deleteWithTimeout
	// usage) and verify killProcessGroup stops it.
