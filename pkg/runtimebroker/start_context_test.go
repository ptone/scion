...
// directory. Agent-1's own worktree is real and writable from inside its own
// container, so agent-1 can create such a subdirectory and later replace it
// with a symlink; only entries that are themselves direct children of
// "worktrees" are ever mounted read-write into any container, so refusing
// anything deeper closes that window.
...
// component — planted inside agent-1's own real worktree, where agent-1 has
// write access — is refused once its physical location is resolved, even
// though the final path component itself is a real, non-symlink directory
// with a gitfile that looks valid on its own.
