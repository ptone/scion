...
func TestGetDefaultSettingsData_ProducesSameEffectiveDefaults(t *testing.T) {
	...
	// Runtime should match what DetectLocalRuntime returns (or OS fallback)
	assert.Equal(t, expectedDefaultRuntime(), settings.Profiles["local"].Runtime)
}
...
func TestDefaultSettingsDataYAML_OSAdjustment(t *testing.T) {
	...
	// Parse as versioned settings to check runtime detection
	...
	assert.Equal(t, expectedDefaultRuntime(), localProfile.Runtime)
}
