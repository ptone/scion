...
- **Auth Proxies and Registration URLs (`register_url`):** If the Scion Hub is deployed behind an authentication proxy, or if users need to register from external networks or mobile devices, you can configure the **Registration URL** (`register_url`) field in the plugin's Integration Settings in the Admin UI. When configured, user-facing registration links generated by the `/register` command will point to this public-facing URL instead of the internal `hub_url`.
...
##### Auth Proxies and Registration URLs (`register_url`)
If the Scion Hub is deployed behind an authentication proxy, or if users need to register from external networks or mobile devices, you can configure the **Registration URL** (`register_url`) field in the plugin's Integration Settings in the Admin UI. When configured, user-facing registration links generated by the `/scion register` command will point to this public-facing URL instead of the internal `hub_url`.
