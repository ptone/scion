...
test_discover_same_pod_cidr_pair_on_other_network_ignored() {
  ...
  # different network -- the stub's own firewall-rules list does not
  # filter by network (it returns every seeded rule regardless), so this
  # exercises discovery's own exact network-URL check: without it, this
  # decoy pair would either be picked directly or make the run ambiguous.
