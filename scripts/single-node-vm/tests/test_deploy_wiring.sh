...
test_deploy_create_tier_off_settings_yaml_byte_identical() {
  ...
  assert_not_contains "$template" "HYBRID_" \
    "the dev-mode heredoc template must carry no hybrid-tier reference outside the three splices"
...
test_deploy_create_tier_off_proxy_settings_yaml_byte_identical() {
  ...
  template="${template%x}"
  assert_not_contains "$template" "HYBRID_" \
    "the proxy-mode heredoc template must carry no hybrid-tier reference outside the three splices"
...
test_deploy_create_tier_on_proxy_settings_yaml_has_shared_dir_storage_block() {
  ...
  assert_eq "1" "$(printf '%s' "$proxy_heredoc" | grep -cx 'default_gcp_identity_mode: passthrough' || true)" \
    "the tier-on proxy-mode write must keep the top-level default_gcp_identity_mode: passthrough line"
...
test_deploy_create_tier_on_reaches_settings_yaml_with_correct_nfs_and_block() {
  ...
  assert_eq "1" "$(printf '%s' "$dev_heredoc" | grep -cx 'default_gcp_identity_mode: passthrough' || true)" \
    "the tier-on dev-mode write must keep the top-level default_gcp_identity_mode: passthrough line"
