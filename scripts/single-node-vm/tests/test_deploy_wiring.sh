...
# The VM create and the router create each take --tags and --description
# from more than one place. Each flag must reach the recorded call at
# most once, so that two code paths adding the same flag cannot combine
# into a duplicate.
...
# or any text between or around the splices, therefore stays in the
# template and fails the comparison with the render. If the fragment is
# not found, the output is an error line, not the template, which also
