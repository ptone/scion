...
# seed_address_extra NAME ADDRESS DESC REGION PROJECT — one more address
# with the same NAME, in REGION of PROJECT's URLs, for the cross-region
# name-collision tests. It is returned only by `compute addresses list`
# (listed before the NAME fixture from seed_address, if any), subject to
# the same region: filter as every other entry; describe, create and
# delete never see it.
seed_address_extra() {
  local name="$1" address="$2" desc="$3" region="$4" project="$5"
  _address_json "$address" "$desc" "INTERNAL" "default" "$region" "$project" \
