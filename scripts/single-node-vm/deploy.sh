...
if ENABLED_APIS="$(gcloud services list --enabled --project="${PROJECT_ID}" \
  ...
  rm -f "${API_LIST_ERR_FILE}"
  # Tier off: this path calls enable unconditionally, which keeps
  # working everywhere, including on a runner that can't list services
  # but can still call enable on an already-enabled API (a harmless
...
#
# Every project-level binding passes --condition=None: without it, gcloud
# refuses to add a binding in non-interactive mode when the project policy
# already contains conditional bindings. gcloud's stderr is captured and
# printed only when the binding fails.
