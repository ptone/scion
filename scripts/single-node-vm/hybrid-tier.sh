...
# The gcloud --filter is only an optimisation; a rule's network is always
# re-checked here against the exact expected URL suffix, so a filter that
# gcloud interprets more loosely than expected (or a stub/real-gcloud
# mismatch) can never let a same-named, same-pod-CIDR rule on a different
# network be mistaken for this cluster's own.
