...
Configure a project-level mode (`assign` with a dedicated service account) for
projects that need GCP access. The hub-level `passthrough` default applies to
hubs deployed without the hybrid tier.
