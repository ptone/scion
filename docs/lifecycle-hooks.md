...
**Status**: Shipping (M1–M5 complete; cross-instance HA de-duplication pending)
...
> **Note**: Cross-instance HA de-duplication is pending an HA-model change.
> The current implementation uses an in-memory deduper for single-instance /
> SQLite deployments and a store-backed atomic compare-and-set for Postgres
> deployments. A future update will integrate with the HA model to ensure
> exactly-once hook execution across multiple Hub instances.
