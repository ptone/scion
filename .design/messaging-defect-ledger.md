...
| DEF-100 | Thread conversations are written with `external_ref=''` but read via derived `thread:<projectID>:<threadID>` — no group thread resolves with the read-switch ON [^16] |  **OPEN** | §5kk | §5kk |
...
None. All 100 ids (DEF-1 through DEF-100) have definitions in the journal.
