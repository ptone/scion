...
This must be measured before the phase lands, not assumed. Two acceptable outcomes: the container image is rebuilt from the same commit as the hub on upgrade, in which case there is no skew and nothing to do; or it is not, in which case the DTO gains a rejected-on-mismatch version field and skew becomes a loud failure instead of a blank one. **Silent zero-valued decode is not an acceptable third outcome.** This is OQ-133-1 and it gates P4.
...
- **OQ-133-1 (gates P4)** — is the agent container's `scion` CLI guaranteed to match the hub build on upgrade? Determines whether a version field is required. My read: assume skew until someone shows me the rebuild, because the failure mode is silent.
- **OQ-133-2** — does any consumer outside this repository read the message DTO? F3 says no spec exists, but absence of a spec is not absence of a consumer, and an assertion that something is absent requires a search. Concretely: check hub access logs on gteam for `GET /api/v1/agents/*/messages` from user-agents that are not the CLI or the web UI. Non-blocking for P1–P3.
