...
| `Attach`, `Sync`, `GetWorkspacePath` | Return errors naming Phase 2 / the hub workspace API. Substrate has no exec/attach/TTY primitive in Phase 1. The broker's PTY switch (`pty_handlers.go`) still rejects a substrate agent's PTY stream internally, but by the time that happens the hub has already upgraded the client's WebSocket connection, so the rejection cannot reach the CLI as a clean error over that connection; it is defence in depth only. `scion attach` itself rejects a substrate agent before any WebSocket dial: `cmd/attach.go`'s `attachViaHub` inspects the agent's runtime from the hub's agent-get response and returns a fixed error ("attach is not supported for agents on the substrate runtime in this phase") for a non-zero CLI exit, the same way it already does for managed agents. |
