...
| BSD `sed` | Rejects the GNU-style `--help` extractor. Probes added (`sed --help`, `sed BRE \?`) but **not yet run on macOS** — awaits first CI run on a branch containing them | Assume BSD `sed`; no GNU-only addressing or BRE extensions (`\?`) |
| BSD `grep` 2.6.0-FreeBSD | Predicted from version string; probe added (`grep -P`) but **not yet run on macOS**. Linux run confirms GNU grep supports `-P` (control) | No `-P` (PCRE not linked) |
| `awk` 20200816 (BWK) | Predicted from version string; probe added (`awk gensub`) but **not yet run on macOS**. Linux run confirms mawk (stock Linux `awk`) also lacks `gensub` | Not `gawk`; no `gensub` |
