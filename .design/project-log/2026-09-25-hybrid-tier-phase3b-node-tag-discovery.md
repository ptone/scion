...
`hybrid_discover`'s node-tag lookup moves from following a cluster's node pools to their managed
instance groups, then to each group's instance template, to reading the tag directly off the
cluster's own GKE-managed firewall rules. On an Autopilot cluster, the node instance groups,
templates and instances are not visible as Compute resources in the project at all -- every
managed-instance-group describe call returns not-found, even for a cluster with running nodes --
so the old path could never succeed there, despite Autopilot being a documented, supported
attach target. The new path reads a source that exists identically for both cluster types.
